...
function actualizarStats() {
  ...
  // Mostrar fecha activa de oferta si existe
  const cfg = JSON.parse(localStorage.getItem("oferta_config") || "{}");
  const el = document.getElementById("oferta-fin-actual");
  if (el) {
    if (cfg.fin) {
      const d = new Date(parseInt(cfg.fin));
      el.textContent = d.toLocaleString("es-MX", { day:"2-digit", month:"2-digit", year:"numeric", hour:"2-digit", minute:"2-digit" });
      el.style.color = Date.now() < cfg.fin ? "#38a169" : "#e53e3e";
    } else {
      el.textContent = "Sin fecha configurada";
      el.style.color = "var(--text-muted)";
    }
  }
}

// ----- GUARDAR FECHA OFERTA (countdown) -----
function guardarFechaOferta() {
  const input = document.getElementById("oferta-fecha-input").value;
  if (!input) { mostrarToast("⚠️ Selecciona una fecha y hora"); return; }
  const fin = new Date(input).getTime();
  if (isNaN(fin) || fin <= Date.now()) { mostrarToast("⚠️ La fecha debe ser futura"); return; }
  localStorage.setItem("oferta_config", JSON.stringify({ fin }));
  actualizarStats();
  mostrarToast("✅ Fecha de oferta guardada");
}

function limpiarFechaOferta() {
  if (!confirm("¿Quitar la oferta activa del countdown?")) return;
  localStorage.removeItem("oferta_config");
  document.getElementById("oferta-fecha-input").value = "";
  actualizarStats();
  mostrarToast("🗑️ Oferta eliminada");
}
...
function enviarPedidoWhatsapp() {
  ...
  // Guardar para botón "reabrir WhatsApp"
  window._ultimaUrlWa = waUrl;

  // Guardar resumen antes de vaciar
  const pedidoItems = [...carrito];
  const pedidoTotal = total;

  // Vaciar carrito
  carrito.forEach(item => {
    const btn = document.getElementById(`btn-${item.id}`);
    if (btn) { btn.classList.remove("added"); btn.innerHTML = `<i class="fa fa-cart-plus"></i> Agregar al carrito`; }
  });
  carrito = [];
  actualizarCarritoUI();
  ...
  cerrarCheckout();
  mostrarConfirmacion(nombre, pedidoItems, pedidoTotal);
}
...
function mostrarConfirmacion(nombre, items, total) {
  const numPedido = "LD-" + Date.now().toString().slice(-6);
  const ahora = new Date();
  const fecha = ahora.toLocaleDateString("es-MX", { day:"2-digit", month:"2-digit", year:"numeric" });
  const hora  = ahora.toLocaleTimeString("es-MX", { hour:"2-digit", minute:"2-digit" });
  ...
  const itemsHTML = items.map(i =>
    `<div class="confirm-item">
      <img src="${i.img}" alt="${i.titulo}" onerror="this.src='https://via.placeholder.com/40x52/f0e8dc/c8860a?text=📚'" />
      <div>
        <p class="confirm-item-titulo">${i.titulo}</p>
        <p class="confirm-item-sub">${i.cantidad}× — $${(i.precio * i.cantidad).toFixed(2)}</p>
      </div>
    </div>`
  ).join("");
  ...
  document.getElementById("confirmModal").innerHTML = `
    <div class="confirm-inner">
      ...
      <h2 class="confirm-titulo">¡Pedido enviado!</h2>
      <p class="confirm-sub">Hola <strong>${nombre}</strong>, tu pedido fue enviado por WhatsApp.<br>Te responderemos pronto con los datos de pago.</p>
      ...
      <p class="confirm-fecha">📅 ${fecha} — ${hora} &nbsp;|&nbsp; 📥 Entrega digital</p>
      <div class="confirm-acciones">
        <button class="confirm-btn-wa" onclick="window.open(window._ultimaUrlWa,'_blank')">
          <i class="fab fa-whatsapp"></i> Abrir WhatsApp
        </button>
        <button class="confirm-btn-seguir" onclick="cerrarConfirmacion()">
          Seguir comprando
        </button>
      </div>
    </div>`;
